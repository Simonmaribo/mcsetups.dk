import getProducts, { Filters, filterToString, stringToFilter } from "@/api/admin/products/getProducts";
...
import { ArrowLeft, CheckCircle2, Download, Eye, Filter, MoreVertical, Search, ShoppingBag, SortDesc } from "lucide-react";
            ...
            <div>
                <div className="bg-white border-b border-gray-200">
                    <div className="mx-auto max-w-screen-2xl px-2.5 md:px-20 pt-10 pb-10 flex items-center gap-4">
                        ...
                        <h1 className="text-2xl font-bold text-slate-900">Produkter på siden</h1>
                    </div>
                </div>
                <div className="mx-auto max-w-screen-2xl px-2.5 md:px-20 pb-10 gap-4">
                    <div className="mt-4 relative w-full mx-auto text-left ring-1 max-w-none bg-white shadow border-blue-400 ring-gray-200 pl-6 pr-6 pt-6 pb-6 rounded-lg">
                            ...
                            <div className="gap-4 flex flex-col md:flex-row mb-8">
                                <div className="bg-white flex flex-1 items-center py-2 px-4 rounded-lg gap-2 group border text-gray-500 border-gray-300 focus-within:text-blue-700 focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-600 focus-within:border-transparent">
                                    ...
                                    <input 
                                        ...
                                        placeholder="Søg efter navn eller brief"
                                        onKeyDown={(e) => {
                                            if(e.key === 'Enter'){
                                                setPage(1);
                                        ...
                                        <PopoverContent className="flex flex-col gap-6">
                                            <div className="flex flex-col gap-3">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <SortButton active={sort == 'mostViews'} text="Flest visninger" onClick={() => applySort('mostViews')}/>
                                                    <SortButton active={sort == 'leastViews'} text="Mindst visninger" onClick={() => applySort('leastViews')}/>
                                                    <SortButton active={sort == 'mostSales'} text="Flest salg" onClick={() => applySort('mostSales')}/>
                                                    <SortButton active={sort == 'leastSales'} text="Mindst salg" onClick={() => applySort('leastSales')}/>
                                                    <SortButton active={sort == 'mostDownloads'} text="Flest downloads" onClick={() => applySort('mostDownloads')}/>
                                                    <SortButton active={sort == 'leastDownloads'} text="Mindst downloads" onClick={() => applySort('leastDownloads')}/>
                                                </div>
                                            </div>
                                        </PopoverContent>
                                    </Popover>
                                    ...
                                    <tbody>
                                        {
                                            products?.products.map((product, i) => (
                                                <tr className="group bg-white border-b hover:bg-gray-50" key={product.id}>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        {
                                                            product.public ? (
                                                                <Badge color="green" size="xs">Offentlig</Badge>
                                                            ) : (
                                                                <Badge color="red" size="xs">Privat</Badge>
                                                            )
                                                        }
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        <div className="flex justify-between items-center">
                                                            <div className="flex gap-4 items-center">
                                                                <div className={cn("flex gap-1 items-center text-gray-700",
                                                                    (sort === 'mostSales' || sort === 'leastSales') ? 'text-blue-700' : 'hover:text-blue-700 cursor-pointer'
                                                                )}>
                                                                ...
                                                                <div className={cn("flex gap-1 items-center text-gray-700",
                                                                    (sort === 'mostDownloads' || sort === 'leastDownloads') ? 'text-blue-700' : 'hover:text-blue-700 cursor-pointer'
                                                                )}>    
                                                                ...
                                                                <div className={cn("flex gap-1 items-center text-gray-700",
                                                                    (sort === 'mostViews' || sort === 'leastViews') ? 'text-blue-700' : 'hover:text-blue-700 cursor-pointer'
                                                                )}>    
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex">
                                                            <Tippy content={<span>{dateToFullString(product.createdAt)}</span>}>
                                                                <p className="text-blue-500 font-medium">{prettyDate(product.createdAt)}</p>
                                                            </Tippy>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4">
...
export default withAuth(Products, { maxGroup: 1})